            ...
            if (interval.get(1, 3600)) { // 1 минута
                if (Tables.playtime.getFields().containsKey(Const.SERVER_COLUMN_NAME)) {
                    for (Player p : Groups.player) {
                        try {
                            Long time = Database.get(p.uuid(), Tables.playtime.getFields().get(Const.SERVER_COLUMN_NAME), Tables.playtime);
                            if (time == null) {
                                Log.err("Player '" + p.uuid() + "' doesn't exists");
                                DiscordLogger.err("Player '" + p.uuid() + "' doesn't exists");
                            }
                            long computed = (time != null ? time : 0) + 60;
                            Database.update(p.uuid(), Tables.playtime.getFields().get(Const.SERVER_COLUMN_NAME), Tables.playtime, computed);
                        } catch (Throwable t) {
